...
## Возможные ошибки
- **Задержка загрузки данных (НЕ ошибка): возможна до 10 сек из-за холодного старта Google Run.** Сразу после клика последнего пункта меню скрипт может показать «Trying to fetch...» / `agRows:0` / «0 to 0 of 0» — данные ещё грузятся. Подождать ~10 сек и повторно проверить состояние страницы (например, через `Runtime.evaluate`: число `.ag-row`, текст пейджера).
