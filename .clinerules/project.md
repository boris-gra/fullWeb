...
- **Действующая договорённость (17.08.2026, подтверждена пользователем): лог вести для КАЖДОГО чата, пока пользователь явно не отменит.** Если договорённость не действует — формировать лог только если пользователь в начале чата попросил вести лог.
...
- **Скилл `open-menu` (основной способ)**: `.clinerules/skills/open-menu.md` — инструкции для Cline + полная копия скрипта. Автоматически подхватывается, когда нужно открыть пункт меню в приложении Query. Если `.cline_scripts/cdp-open-menu.ps1` отсутствует — воссоздать его из кода в скилле.
